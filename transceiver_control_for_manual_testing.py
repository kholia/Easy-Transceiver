#!/usr/bin/env python3
# pylint: disable=missing-function-docstring,invalid-name,wrong-import-position,missing-module-docstring
...
configs = yaml.load(configs_file, Loader=yaml.SafeLoader)
        ...
        while True:
            ...
            if 'FT8' in mode:
                if utc_time.second % 15 == 14:
                    print("TX!")
                    time.sleep(0.9)  # for arduino nano code path
                    puerto.write(b't')
                    time.sleep(1)
                    break
            else:
...
# Create the menus
menu = ConsoleMenu("WSJTX Transceiver Control ", "FT8 and FT4")
